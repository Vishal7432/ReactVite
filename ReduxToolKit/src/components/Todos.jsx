...
function Todos() {
  ...
  return (
    <>
      <div className=" flex justify-center text-2xl text-blue-500 font-bold ml- mt-5">
        Todos
      </div>
      <ul className="list-none  max-w-md mx-auto">
        {todos.map((todo) => (
          <li
            ...
            key={todo.id}
          >
            {isEditing && currentEditId === todo.id ? (
              <input
                value={editText}
                onChange={(e) => setEditText(e.target.value)}
                className="text-white bg-gray-800 px-2 py-1 rounded flex-1 mr-4 outline-none border border-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-900 transition-colors duration-200 ease-in-out"
              />
            ) : (
            ...
            <div className="flex gap-2">
              {isEditing && currentEditId === todo.id ? (
                <button
                  onClick={() => {
                    dispatch(
                      upDateTodo({ id: currentEditId, title: editText }),
                    );
                    setIsEditing(false);
